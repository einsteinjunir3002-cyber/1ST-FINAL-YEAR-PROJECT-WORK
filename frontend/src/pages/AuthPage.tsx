...
import { Lock, Mail, User, Sparkles, Building, Bookmark } from 'lucide-react';
...
export const AuthPage: React.FC = () => {
  ...
  const [password, setPassword] = useState(() => localStorage.getItem('preferred_email') ? 'password123' : '');
          ...
          <form onSubmit={handleAuthSubmit} className="flex flex-col gap-4">
                ...
                >
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`flex-1 py-2.5 border rounded-xl text-[11px] font-bold transition-all cursor-pointer ${role === 'student' ? 'border-violet-600 bg-violet-600/10 text-violet-400' : 'border-slate-800 bg-slate-950 text-slate-500'}`}
                    >
                      Student
                    </button>
                    <button
                      ...
                      className={`flex-1 py-2.5 border rounded-xl text-[11px] font-bold transition-all cursor-pointer ${role === 'lecturer' ? 'border-violet-600 bg-violet-600/10 text-violet-400' : 'border-slate-800 bg-slate-950 text-slate-500'}`}
                    >
                      Lecturer
                    </button>
                    <button
                      ...
                      className={`flex-1 py-2.5 border rounded-xl text-[11px] font-bold transition-all cursor-pointer ${role === 'admin' ? 'border-violet-600 bg-violet-600/10 text-violet-400' : 'border-slate-800 bg-slate-950 text-slate-500'}`}
                    >
                      Administrator
                    </button>
                  </div>
            ...
            {activeTab === 'signin' && (
              <button
                ...
                disabled={loading}
                className="w-full mt-2 py-3.5 rounded-xl border border-slate-800 hover:border-slate-700 bg-slate-950 text-slate-300 font-bold text-xs shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer hover:bg-slate-900"
              >
                🔑 Sign In with Device Biometrics (Face ID/Fingerprint)
              </button>
            )}
          </form>
