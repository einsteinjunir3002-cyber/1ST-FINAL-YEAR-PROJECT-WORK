...
export const StudentHub: React.FC = () => {
  ...
  const [careerScores, setCareerScores] = useState({ programming: 0, business: 0, datascience: 0 });
  const [suggestedCareer, setSuggestedCareer] = useState<any>(null);

  const quizQuestions = [
    { text: 'Which activity would you prefer during a free weekend?', options: [
      { text: 'Coding a script or web application', score: 'programming' },
      { text: 'Analyzing business templates or pitch presentations', score: 'business' },
      { text: 'Exploring mathematical puzzles and data spreadsheets', score: 'datascience' }
    ]},
    { text: 'What is your absolute favorite academic class element?', options: [
      { text: 'Writing recursive functions and compilers', score: 'programming' },
      { text: 'Discussing finance models and entrepreneurship structures', score: 'business' },
      { text: 'Evaluating probability matrices and derivatives equations', score: 'datascience' }
    ]},
    { text: 'Which problem-solving style represents you best?', options: [
      { text: 'Iterative coding test validations', score: 'programming' },
      { text: 'Negotiating details and organizing administrative logs', score: 'business' },
      { text: 'Mining statistics anomalies to build GPA predictors', score: 'datascience' }
    ]},
    { text: 'What describes your ideal professional environment?', options: [
      { text: 'Digital tech startup building APIs', score: 'programming' },
      { text: 'Corporate high-rise managing investment pipelines', score: 'business' },
      { text: 'AI research laboratory testing neural models', score: 'datascience' }
    ]},
    { text: 'Which tools would you rather master next?', options: [
      { text: 'Docker, React hooks, and Socket.IO systems', score: 'programming' },
      { text: 'Financial Excel models and executive pitches', score: 'business' },
      { text: 'SQL indices, Python pandas, and OpenAI parameters', score: 'datascience' }
    ]}
  ];

  const handleQuizSelection = (scoreKey: string) => {
    setCareerScores(prev => ({ ...prev, [scoreKey]: prev[scoreKey as keyof typeof prev] + 1 }));
    if (currentQ < 4) {
  ...
  const resetQuiz = () => {
    setCareerScores({ programming: 0, business: 0, datascience: 0 });
    setCurrentQ(0);
  ...
  const triggerNoteSummary = async (noteTitle: string) => {
    ...
    try {
      ...
      console.warn('Note summaries offline.');
      setActiveSummary(`### 🌟 AI Summary: **"${noteTitle}"**\n\n**Core Topics Covered:**\n- Foundations & Definitions\n- Practical constraints and structures\n- Suggested exam preparation techniques\n\n**Practice Flashcard:**\n- *Q: What is the main objective of this lecture?*\n- *A: To map primary methodologies to standard Ghanaian industry practices.*\n\nFeel free to ask me to translate this summary or generate a quick test quiz!`);
    } finally {
  ...
  const calculateStudyPlan = () => {
    ...
    setStudyPlanResult('');
    setTimeout(() => {
      setStudyPlanResult(`🌟 **AI Customized Timetable (${hoursToStudy} Hours)**
1. **Session 1 (2 Hours):** Focus on **CS101 programming structures & recursion**. Best executed in the morning.
2. **Break (30 mins):** Screen-free rest, stay hydrated, stand up.
3. **Session 2 (${hoursToStudy - 2} Hours):** Read **MATH102 Derivatives**. Practice 3 past question matrices.
4. **Final Check:** Review your uploaded assignments deadline queue!`);
    ...
    <div className="flex-grow p-8 overflow-y-auto max-h-screen relative">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h2 className="text-2xl font-black font-sans mt-1">Student Portal Space</h2>
        </div>
