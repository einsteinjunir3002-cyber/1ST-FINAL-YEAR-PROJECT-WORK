...
// Cryptographic Biometric Challenge Cache
const biometricChallenges = new Map<string, { challenge: string; expiresAt: number }>();

export const getBiometricRegisterOptions = async (req: AuthenticatedRequest, res: Response) => {
  if (!req.user) return res.status(401).json({ message: 'Unauthorized' });
  
  const userId = req.user.id;
  const challenge = crypto.randomBytes(32).toString('hex');
  
  biometricChallenges.set(`register:${userId}`, {
    challenge,
...
export const verifyBiometricRegister = async (req: AuthenticatedRequest, res: Response) => {
  if (!req.user) return res.status(401).json({ message: 'Unauthorized' });
  
  const userId = req.user.id;
  const { publicKey, signature, keyId } = req.body;
  
  if (!publicKey || !signature || !keyId) {
    return res.status(400).json({ message: 'Public key, signature and keyId are required.' });
  }
  
  const challengeData = biometricChallenges.get(`register:${userId}`);
  if (!challengeData || challengeData.expiresAt < Date.now()) {
    return res.status(400).json({ message: 'Challenge expired or invalid.' });
  }
  
  biometricChallenges.delete(`register:${userId}`);
  
  try {
    const verifier = crypto.createVerify('SHA256');
    verifier.update(Buffer.from(challengeData.challenge));
    ...
    if (!isValid) {
      return res.status(400).json({ message: 'Invalid biometric signature verification.' });
    }
    ...
    user.biometricKeyId = keyId;
    await user.save();
    
    return res.status(200).json({ message: 'Biometrics registered successfully!' });
  } catch (err) {
    console.error('Biometric register verification error:', err);
    return res.status(500).json({ message: 'Error verifying biometric registration.' });
  }
};
...
export const getBiometricLoginOptions = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!user.biometricPublicKey) {
      return res.status(400).json({ message: 'Biometrics not set up for this account.' });
    }
    
    const challenge = crypto.randomBytes(32).toString('hex');
    biometricChallenges.set(`login:${email.toLowerCase().trim()}`, {
      challenge,
      expiresAt: Date.now() + 5 * 60 * 1000
    });
    ...
    return res.status(200).json({
      challenge,
      keyId: user.biometricKeyId
    });
  } catch (err) {
    console.error('getBiometricLoginOptions error:', err);
    return res.status(500).json({ message: 'Server error retrieving biometrics options.' });
  }
};
...
export const verifyBiometricLogin = async (req: Request, res: Response) => {
  ...
  const challengeData = biometricChallenges.get(`login:${email.toLowerCase().trim()}`);
  if (!challengeData || challengeData.expiresAt < Date.now()) {
    return res.status(400).json({ message: 'Challenge expired or invalid.' });
  }
  
  biometricChallenges.delete(`login:${email.toLowerCase().trim()}`);
  
  try {
    const user = await User.findOne({ email: email.toLowerCase().trim() });
    if (!user || !user.biometricPublicKey) {
      return res.status(400).json({ message: 'Biometrics not set up or user not found.' });
    }
    ...
    return res.status(200).json({
      message: 'Signed in successfully with biometrics!',
      token,
      user: {
        id: user._id.toString(),
        name: user.name,
    ...
    return res.status(500).json({ message: 'Internal server error during biometric signin.' });
  }
};
